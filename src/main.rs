...
fn main() {
    let entered_address = dotenv::var("WALLET").expect("Error reading env file");
...
fn blockchain_info_app(address: &str) {
    ...
    println!(
        "\n\nAnalyzing transactions for {} address {}",
        &blockchain_status.blockbook.coin, &blockchain_address.address
    );
    ...
    println!("\n Do you want to query this transactions? (y/n)\n");
